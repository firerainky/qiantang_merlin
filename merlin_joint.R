...
df_long <- df %>%
  ...
  select(number, age, mtx.date, time, hcg, log_hcg, menopause_days, status, time.in.hospital) 
...
df_merged <- df_long %>%
  left_join(
    df_with_slope %>% select(number, stime, hcg_slope),
    by = "number"
  )
...
predict(m3, stat = "survival", type = "marginal", model = 1, at = c("stime" = 9, "hcg_slope" = 0.12))
